"""test_v13_structural.py — ST1/ST2/ST3 structural tests for v1.3.

Identical assertions to test_v12_structural.py — same source-code
inspection, same forbidden symbols. The point is precisely that the
invariant DOES NOT CHANGE between proxy implementations: the v1.3
Apache integration must inherit the v1.2 architectural guarantee
that FastAPI has zero auth code, regardless of which reverse proxy
sits in front.

If you're tempted to add an Apache-specific allowlist / config /
header trust to FastAPI in v1.3 (e.g., 'NGINX_MODE was nginx-only,
APACHE_MODE is the Apache equivalent…'), STOP — that's the v1.1
hybrid pattern reincarnated under a different proxy. ST2/ST3 here
catch it the same way they catch any v1.2 regression.
    ...
    ST3   server.py contains no TLS setup, no cert parsing, and does
          not read any X-Client-* header — those are the proxy's concern,
          regardless of whether the proxy is nginx (v1.2) or Apache (v1.3).
...
Run:
    pytest tests/test_v13_structural.py -v

Why duplicate test_v12_structural.py at all? Two reasons:
  1. The test IDs (test_st1_*, test_st2_*, test_st3_*) need to appear
     in CI under a v1.3 module name so the cross-architecture coverage
     table shows that v1.3 inherits the same structural guarantees.
  2. Future v1.4/etc. integrations can extend this file with proxy-
     specific assertions without polluting the v1.2 baseline.
"""
# ruff: noqa: F811
